...
ipcMain.on(Constant.GET_WBC_HIST_LIST, (event, payload) => {
  var params = JSON.parse(payload)
  // log.info(params)
  //{ cassetId: '20240103122546_', slotId: '20240103122546_02_20240103122938' }
...
ipcMain.on(Constant.GET_TEST_HISTORY_LIST, (event, payload) => {
  ...
  // TEST TYPE 필터링 하려고 추가한 부분
  var tmpStr2 = ''
  if (params.testTypeList.length > 0) {
    params.testTypeList.forEach(function(value, index) {
      tmpStr2 += `'` + value + `'`

      if ((params.testTypeList.length - 1) !== (index)) {
        tmpStr2 += ','
      }
    })

    log.info(tmpStr2)
    searchQuery += '\n AND TA.TEST_TYPE IN (%s)'
    searchQuery = searchQuery.replace('%s', tmpStr2)
  }
