...
const mutations = {
  ...
  UPDATE_NEW_TEST_SET (state, obj) {
    state.settings.newRSelected = obj.newRSelected
    state.settings.newSSelected = obj.newSSelected
    state.settings.newStitchCnt = obj.newStitchCnt
  },
  UPDATE_EXEC_TEST_TYPE (state, value) {
    state.settings.rSelected = value
  },
  INITIALIZE_MOUNT_CNT (state) {
    state.settings.mountCnt = 0
  },
  UPDATE_MOUNT_CNT (state, cnt) {
    cnt++
    state.settings.mountCnt = cnt
  },
  ...
  UPDATE_ANALYSIS_TYPE (state, obj) {
    ...
    // pbp 추가
    state.settings.pbAnalysisType2 = obj.pbAnalysisType2
...
const actions = {
  ...
  updateNewTestSet: (context, value) => {
    context.commit('UPDATE_NEW_TEST_SET', value)
  },
  updateExecTestType: (context, value) => {
    context.commit('UPDATE_EXEC_TEST_TYPE', value)
  },
  updateMountCnt: (context, value) => {
    context.commit('UPDATE_MOUNT_CNT', value)
  },
  // mount 횟수 0으로 초기화
  initializeMountCnt: (context) => {
    context.commit('INITIALIZE_MOUNT_CNT')    
  },
  // mount 횟수 구분하려는 용도
  updateMountCnt: (context, cnt) => {
    context.commit('UPDATE_MOUNT_CNT', cnt)    
  },
