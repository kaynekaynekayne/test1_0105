...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      // this.EventBus.$on('HOME_DESTROY', function(params) {
      //   console.log(params) //self.selectedItem
      ...
      ipcRenderer.on(Constant.GET_NORMAL_RANGE, function (event, results) {
        ...
        // TypeError: Cannot read property 'forEach' of null 떴을 때 주석처리 하면 에러 알림 없어지긴 함
      ...
      worker.norWorker.onmessage = function (event) {
        ...
        if (event.data.params.resultCd === '00000') {
          ...
          if (!self.isModalShow) {
            // 계속 뜨는 에러 알람 안 뜨게 하려는 용 (테스트... 빌드 전)
    ...
    methods: {
      ...
      calcRbcDegree (completeSlot) {
        ...
        log.info('************')
        log.info('************')
        log.info('calcRbcDegree')
        log.info(completeSlot)
        ...
        log.info('chromiaTotal : ' + chromiaTotal)
        log.info('totalCount : ' + totalCount)
        log.info(self.rbcDegreeStandard)
          ...
          rbcCategory.classInfo.forEach(function (rbcClass, clsIndex) {
            self.rbcDegreeStandard.forEach(function(degreeStandard) {
              if ((degreeStandard.CATEGORY_ID === rbcCategory.categoryId) &&
                ...
                log.info('rbc category : ' + rbcCategory.categoryId)
                log.info('rbc class : ' + rbcClass.classId)
      ...
      // 주석처리 해야 immersion oil count reset 정상적으로 나옴
      // onImmersionOil (evt) {
      ...
      commonReceive (params) {
        ...
        if (jobCmd === 'INIT') {
          ...
          if (self.getSettings.isAlarm) {
              if (Number(self.getSettings.alarmCount) === 0) {

                  // 알람이 0초로 설정된 경우 계속 울림
                  ...
                  // 일정 시간 후에 알람 정지
                  setTimeout(function() {
                      clearInterval(self.alarmIntervalId);
                      clearInterval(self.alarmFlickId);
