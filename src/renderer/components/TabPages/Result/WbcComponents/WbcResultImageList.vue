<template>
  <b-container id="wbcResultImageList" fluid @keyup="onKeyUp">
    ...
    <b-row>
      <b-col class="pl-0 pr-0"> <!--여기부터 스크롤 영역-->
        <b-card id="wbcImageCard" @wheel="handleWheel" style="height: 700px; overflow-y:auto;" ref="scrollContainer" class="blackCard" text-variant="white" no-body>
          <div id="wbcImageCardWrapper" style="height: 1000px">
    ...
    methods: {
      ...
      handleWheel (event) {
        if (event.shiftKey) {
          ...
          const scrollStep = 50
          this.$refs.scrollContainer.scrollTop += delta > 0 ? scrollStep : -scrollStep


        }
          // event.preventDefault()

          // const delta = Math.sign(event.deltaY); // deltaY가 양수면 아래로, 음수면 위로
          const delta = event.deltaY
          console.log(delta)
          // 올라갈 때 -100 / 내려갈 때 100
      ...
      loadMarkerPosition (evt) {
        ...
        if (this.selectedItem.TEST_TYPE === '01' || this.selectedItem.TEST_TYPE === '04') {
          ...
          // D:/IA_Proc/20230918171101_00_20230918171108/01_WBC_Classification/20230918171101_00_20230918171108.json
        } else {
      ...
      drawCellMarker () {
        ...
        // (104) [{...},{...}]
        // FILE_NM:"LY_20231016151225_005_000.bmp"
        // ORG_HEIGHT:"240"
        // ORG_WIDTH:"240"
        // POSX1:"57"
        // POSX2:"183"
