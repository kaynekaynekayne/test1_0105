...
<script>
  ...
  export default {
    ...
    beforeDestroy () {
      ...
      // this.EventBus.$off('HOME_DESTROY')
    ...
    mounted () {
      ...
      this.$nextTick(function() {
          ...
          fs.readdir(self.settings.pbiaRootPath + '/' + self.selectedItem.SLOT_ID + '/' + self.settings.barcodeDirName, function(err, files) {
            if (!err) {
              ...
              setTimeout(function() {
                ...
                // to home
                // self.EventBus.$emit('HOME_DESTROY', self.selectedItem)
        ...
        ipcRenderer.on(Constant.SELECT_LIS_CODE, function (event, result) {
          ...
          if (self.siteCd === '0019') { // 뷰어에도 적용해야함 !! !!!!!
          // if (self.siteCd) { // 이거 쓰면 주석 원래대로 돌려놔야됨
          ...
          // 고대 안암
            ...
            if (self.lisConnObj !== null) {
              ...
              // CBC_CONN_TYPE_CD:"01", CBC_FILE_PATH:"", CBC_URL:"", DEVICE_BARCODE:"devBarcode", LIS_CONN_TYPE_CD:"01", LIS_FILE_PATH:"C:/Users/UIMD/IA_MSG/LIS", LIS_URL:""
              ...
              result.forEach(function(lisCode) {
                if (lisCode.LIS_CD !== '') {
                  self.wbcInfo.forEach(function(wbcItem) {
                    ...
                    // console.log('wbcItem')
                    // count: 88, dirName: "71_NS", id: "71", images: Array(88), name: "Neutrophil-Segmented", order: 2, percent: 46, title: "NS"
    ...
    methods: {
      ...
      getCbcResults (isToast) {
        ...
        if (self.siteCd === '0002') {
          ...
          // getKumcCbcPath -> C:/Users/user/Desktop/IA_MSG/CBC
          // fs.readFile("C:/Users/theyg/OneDrive/바탕 화면/IA_MSG/CBC" + '/' + self.selectedItem.BARCODE_NO + '.txt', function(err, data) {
            if (err) {
              console.log(err)
              if (isToast) {
                self.$toasted.show(err.message, {
